...
@section('content')
    ...
    @can('country_create')
    <p>
        <a href="{{ route('admin.roomcategories.create') }}" class="btn btn-success">@lang('quickadmin.qa_add_new')</a>
    ...
    <p>
        <ul class="list-inline">
            <li><a href="{{ route('admin.roomcategories.index') }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('quickadmin.qa_all')</a></li> |
            <li><a href="{{ route('admin.roomcategories.index') }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">@lang('quickadmin.qa_trash')</a></li>
        </ul>
    </p>
    @endcan
                    ...
                    @if (count($categories) > 0)
                        @foreach ($categories as $country)
                            <tr data-entry-id="{{ $country->id }}">
                                ...
                                <td>
                                    @can('country_view')
                                    <a href="{{ route('admin.roomcategories.show',[$country->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.qa_view')</a>
                                    @endcan
                                    @can('country_edit')
                                    <a href="{{ route('admin.roomcategories.edit',[$country->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('country_delete')
{!! Form::open(array(
                                        'style' => 'display: inline-block;',
